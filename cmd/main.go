...
func main() {
	...
	var cacheDefaultCleanupInterval time.Duration
	...
	flag.DurationVar(&cacheDefaultTTL, "cache-default-ttl", 15*time.Minute,
		"The duration for the items in the cache to expire (by default)")
	flag.DurationVar(&cacheDefaultCleanupInterval, "cache-default-cleanup-interval", time.Minute,
	...
	case providerAWS:
		var err error
		pvd, err = aws.NewProvider(cacheDefaultTTL, cacheDefaultCleanupInterval)
		if err != nil {
