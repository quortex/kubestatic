...
func (v *FirewallRuleCustomValidator) validate(ctx context.Context, firewallrule *v1alpha1.FirewallRule) (admission.Warnings, error) {
	...
	if nodename != "" {
		...
		if frSpec.Direction == provider.DirectionIngress &&
			...
			allErrs = append(allErrs, field.Invalid(path, firewallrule.Spec, "ingress rule already exists."))
			...
			allErrs = append(allErrs, field.Invalid(path, firewallrule.Spec, "egress rule already exists."))
		}
	}
